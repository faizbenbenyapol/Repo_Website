...
3. **ล็อกอินด้วยบัญชีในระบบเอง และกุญแจของสมาชิกไม่ลงฐานข้อมูล** (ดู [ADR 0002](docs/decisions/0002-where-the-api-key-lives.md))
   - อีเมล + รหัสผ่านแฮชด้วย scrypt เลือกทางนี้เพราะทดสอบครบวงจรใน Docker แบบปิดเน็ตได้
     ส่วน GitHub OAuth ยังเปิดทางไว้ให้เพิ่มทีหลังบนตารางผู้ใช้เดิม
   - API key อยู่ใน Redis ที่ทำงานในหน่วยความจำล้วน เข้ารหัส AES-256-GCM หมดอายุใน 12 ชั่วโมง
4. **ค้นคืนบริบทของถาม–ตอบใช้ pg_trgm ก่อน ยังไม่ลงทุนกับ pgvector** (ดู [ADR 0003](docs/decisions/0003-trigram-retrieval-before-vectors.md))
   - สิ่งที่ต้องค้นคือชื่อไฟล์ ชื่อฟังก์ชัน และหัวข้อสรุปที่มีอยู่แล้ว ซึ่งเป็นข้อความสั้นที่ตรงตัวอักษรได้ระดับหนึ่ง
   - เพิ่มทีหลังได้โดยไม่ต้องรื้อของเดิม ถ้าพบว่า pg_trgm ไม่พอสำหรับคำถามที่ไม่มีคำตรงกับโค้ดเลย
5. **ส่งออกรายงานภาษาไทยเป็น Markdown ก่อน ยังไม่ทำ PDF ในรุ่น v0.7.0**
   - `pdfkit` (ทางเลือกที่เบาที่สุด) มีแค่ฟอนต์ละติน ไม่รองรับภาษาไทยเลย
   - ทางที่ถูกต้องคือเรนเดอร์ HTML → PDF ด้วยเบราว์เซอร์จริง ซึ่งต้องเพิ่ม Chromium เข้าอิมเมจ API
     และต้องหาไฟล์ฟอนต์ไทยมาฝังเอง เป็นการตัดสินใจด้านโครงสร้างพื้นฐานที่ต้องรอเจ้าของงานเลือกทาง

### ยังต้องการข้อมูลจากเจ้าของงาน

- สเปกเซิร์ฟเวอร์ Ubuntu และมี reverse proxy เดิมอยู่แล้วหรือไม่
- DNS ของ repo.benyapol.com ชี้มาที่เครื่องปลายทางหรือยัง
